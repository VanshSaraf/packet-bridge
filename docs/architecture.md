...
The networking layer owns cross-platform socket setup, endpoint conversion, low-level socket helpers, TCP client/server wrappers, and a UDP socket wrapper. Discovery uses UDP broadcast packets to announce local peers, while transfer uses TCP sessions for manifest exchange, chunk streaming, and SHA-256 integrity verification. Future protocol work will add acknowledgements and checkpoint requests.
...
receiver -> byte-count verification and local SHA-256 comparison
```

The receiver sanitizes the incoming filename, writes the output as `received_<filename>`, validates each chunk before accepting the payload, and reports whether the received count matches the manifest. It then computes SHA-256 for the saved file and compares it with the sender-provided digest. Sender and receiver both report bytes transferred, chunk counts, elapsed time, transfer speed, and ETA.
